...
export default function CoffeeCard({
  ...
  return (
    <article className="group relative w-full max-w-[360px] pt-20 sm:pt-24 md:max-w-[320px] lg:max-w-[340px] [perspective:1200px]">
      {image && (
        <img
          src={image}
          alt={imageAlt || name}
          className={`pointer-events-none absolute left-1/2 top-[-14px] z-20 w-[46%] max-w-[130px] -translate-x-1/2 drop-shadow-[0_18px_24px_rgba(0,0,0,0.28)] transition duration-500 group-hover:-translate-y-2 group-hover:scale-105 sm:top-[-18px] sm:w-[52%] sm:max-w-[145px] lg:max-w-[150px] ${imageClassName}`}
        />
      )}

      <div className="h-[240px] sm:h-[252px] lg:h-[260px]">
        <div className="relative h-full rounded-lg transition-transform duration-700 [transform-style:preserve-3d] group-focus-within:[transform:rotateY(180deg)] group-hover:[transform:rotateY(180deg)]">
          <div className="absolute inset-0 overflow-hidden rounded-lg border border-white/65 bg-white/20 p-4 pt-14 shadow-[inset_0_1px_0_rgba(255,255,255,0.75),0_22px_58px_rgba(74,13,13,0.18)] backdrop-blur-[28px] [backface-visibility:hidden] sm:p-5 sm:pt-16">
            ...
            <div className="relative z-10 mb-4 flex items-center justify-between gap-4 sm:mb-5">
              <div className="flex h-10 w-10 items-center justify-center rounded-full border border-white/70 bg-[#721111]/75 text-[#F0E5D1] shadow-[inset_0_1px_0_rgba(255,255,255,0.25),0_10px_24px_rgba(74,13,13,0.2)] backdrop-blur-xl sm:h-11 sm:w-11">
                <Coffee size={18} />
              </div>
              <span className="rounded-full border border-white/70 bg-white/30 px-3 py-1 text-sm font-bold text-[#721111] shadow-[inset_0_1px_0_rgba(255,255,255,0.65)] backdrop-blur-2xl">
                {price}
              </span>
            </div>

            <h3 className="relative z-10 text-base font-semibold text-[#4A0D0D] sm:text-lg">
              {name}
            </h3>
            <p className="relative z-10 mt-2 line-clamp-3 text-sm leading-6 text-[#4A0D0D]/68 sm:mt-3">
              {note}
            </p>
            <span className="absolute bottom-4 left-4 z-10 text-[11px] font-semibold uppercase tracking-[0.18em] text-[#721111]/65 sm:bottom-5 sm:left-5 sm:text-xs">
              Flip
            </span>
          </div>
          ...
          <div className="absolute inset-0 flex flex-col justify-between overflow-hidden rounded-lg border border-white/70 bg-[#721111]/80 p-4 pt-14 text-[#F0E5D1] shadow-[inset_0_1px_0_rgba(255,255,255,0.28),0_22px_58px_rgba(74,13,13,0.22)] backdrop-blur-[30px] [backface-visibility:hidden] [transform:rotateY(180deg)] sm:p-5 sm:pt-16">
            ...
            <div className="relative z-10">
              ...
              <p className="mt-3 line-clamp-4 text-sm leading-6 text-[#F0E5D1]/78 sm:mt-4 sm:leading-7">
